...
test("quality pass 12 preserves exact official release months on two 3 Oaks cascade records", () => {
  ...
  for (const [slug, values] of Object.entries(expected)) {
    ...
    expect(research?.mechanics, `${slug} must preserve its original cascade mechanic`).toContain("Каскады");
