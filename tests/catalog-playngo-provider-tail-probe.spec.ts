...
test("probe remaining Play’n GO score-2 composition", () => {
  const rows = catalogSeeds
    .filter((seed) => seed.provider === "Play’n GO" && scoreFor(seed.slug) === 2)
    .map((seed) => ({
      slug: seed.slug,
      name: seed.name,
      source: seed.source,
  ...
  console.log(`PLAYNGO_SCORE2=${JSON.stringify(rows)}`);
});
