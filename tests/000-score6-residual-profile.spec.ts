import { expect, test } from "@playwright/test";
import { catalogSeeds } from "../src/lib/catalog-seeds";
import { getVerifiedCatalogDetails } from "../src/lib/catalog-verified-details-lookup";
import { getVerifiedCatalogGameType } from "../src/lib/catalog-verified-game-type";
import { getVerifiedCatalogResearch } from "../src/lib/catalog-research-lookup";

function profile(slug: string) {
  const details = getVerifiedCatalogDetails(slug);
  const type = getVerifiedCatalogGameType(slug);
  const research = getVerifiedCatalogResearch(slug);
  const facts = [details?.field, details?.rtp, details?.maxWin, details?.volatility, details?.releaseDate];
  const score = facts.filter(Boolean).length + (type ? 1 : 0) + (research?.mechanics.length ?? 0);
  return {
    score,
    missing: ["field", "rtp", "maxWin", "volatility", "releaseDate"].filter((_, i) => !facts[i]),
    gameType: type?.gameType,
    mechanics: research?.mechanics ?? [],
    source: details?.source,
  };
}

test.only("profile post-bulk score-six residual", () => {
  const residual = catalogSeeds
    .map((seed) => ({ provider: seed.provider, slug: seed.slug, ...profile(seed.slug) }))
    .filter((row) => row.score === 6);

  const byProvider = residual.reduce<Record<string, number>>((acc, row) => {
    acc[row.provider] = (acc[row.provider] ?? 0) + 1;
    return acc;
  }, {});

  console.log("GLOBAL_SCORE6_PROVIDER_COUNTS", JSON.stringify(byProvider));
  console.log("GLOBAL_SCORE6_RESIDUAL", JSON.stringify(residual));
  expect(residual.length).toBe(-1);
});
