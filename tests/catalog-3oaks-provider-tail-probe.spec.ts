...
test("probe remaining 3 Oaks score-2 composition", () => {
  const rows = catalogSeeds
    .filter((seed) => seed.provider === "3 Oaks Gaming" && scoreFor(seed.slug) === 2)
    .map((seed) => ({
      slug: seed.slug,
      name: seed.name,
      source: seed.source,
  ...
  console.log(`THREE_OAKS_SCORE2=${JSON.stringify(rows)}`);
});
