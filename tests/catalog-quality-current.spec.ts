...
test("current catalog quality baseline", () => {
  ...
  expect(ranked.filter((row) => row.score === 3)).toHaveLength(604);
  ...
  expect(ranked.some((row) => row.slug === "playn-go-coin-club")).toBe(false);
  expect(ranked.some((row) => row.slug === "playn-go-rally-4-riches" && row.score === 3)).toBe(true);
});
