...
test.only("profile remaining Play’n GO passport gaps after batch 11", () => {
  ...
  throw new Error(`PLAYNGO_PROFILE ${JSON.stringify({ totalIncomplete: rows.length, byField, first60: rows.slice(0, 60) })}`);
});
